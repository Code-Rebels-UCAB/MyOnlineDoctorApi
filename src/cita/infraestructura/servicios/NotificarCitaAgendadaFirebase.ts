...
export class NotificarCitaAgendadaFirebase implements IPolitica<string,void>{
    public constructor(
    ...
    async ejecutar(data: string): Promise<Resultado<void>> {
      ...
      admin.app(); // if already initialized, use that one
        ...
        const payload = {
          notification: {
            title: 'Cita Agendada',
            body: `${gender2} ${doctorT.p_apellido} te agendó una cita para el ${citaT.fechacita} a las ${citaT.horacita}`,
            image: 'https://pbs.twimg.com/media/FY3c_ZXWQAArjd1?format=png&name=small'
          },
          data:{ 
            info: `Title:doctor agenda cita, test`,
          }
        };
