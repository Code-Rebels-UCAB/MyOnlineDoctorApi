...
export class AceptarCita implements IServicioAplicacion<string,CitaDataDTO>
{
    public constructor(
    ...
    async ejecutar(datos: string): Promise<Resultado<CitaDataDTO>> {
        try{
            ...
            var CitaAceptada = CitaMapeador.covertirInfraestructuraAplicacion(citaActualizada);
            return Resultado.Exito<CitaDataDTO>(CitaAceptada);
        }
        catch (error) {
